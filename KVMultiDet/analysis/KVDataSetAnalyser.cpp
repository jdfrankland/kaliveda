...
void KVDataSetAnalyser::ChooseSystem(const Char_t* data_type)
{
   ...
   if (!fDataSet->GetDataBase()->GetSystems()->GetEntries()) {
      fChoozRuns = kTRUE;
      return;
   }
...
KVNumberList KVDataSetAnalyser::PrintAvailableRuns(KVString& datatype)
{
   ...
   all_runs.Begin();
   while (!all_runs.End()) {
      dbrun = fDataSet->GetDataBase()->GetRun(all_runs.Next());
      if (dbrun) {
...
void KVDataSetAnalyser::SetRuns(const KVNumberList& nl, Bool_t check)
{
   ...
   if (!check) {
      ...
      KVDBRun* run = fDataSet->GetDataBase()->GetRun(nl.First());
   ...
   while (!nl.End()) {
      ...
      KVDBRun* run = fDataSet->GetDataBase()->GetRun(run_no);
      if (!i) {
